...
class GraphState(TypedDict):
    ...
    Attributes:
        ...
        failed_message: message of failure
        failed_search_by_not_relevant: count of failed search by not relevant
        failed_generate_by_hallucination: count of failed generated by hallucination
    """
    ...
    failed_message: str
    failed_search_by_not_relevant: bool
    failed_generate_by_hallucination: bool
...
def decide_to_generate(state: GraphState):
    ...
    if need_web_search == "Yes":
        ...
        if not state.get("failed_search_by_not_relevant", default=False):
            state["failed_message"] = "failed: not relevant"
            return "not_found_relevant"
        else:
            state["failed_search_by_not_relevant"] = True
            return "websearch"
    else:
...
def grade_generation_v_documents_and_question(state: GraphState):
    ...
    if is_exist:
        ...
        if not state.get("failed_generate_by_hallucination", default=False):
            state["failed_generate_by_hallucination"] = True
            return "not supported"
        else:
            state["failed_message"] = "failed: not hallucination"
            return "exist_hallucination"
